...
pub fn assembly_for_ir_item(ir_item: &IRItem) -> String {
    ...
    let dump_ir_type = |ir_type: &IRType| {
        format!("({} {})", ir_type.size, ir_type.align)
    };

    let dump_symbol = |symbol: &Symbol| {
        SessionGlobals::with_interner(|interner| interner.get(symbol).unwrap())
    };

    let dump_ir_value = |ir_value: &IRValue| {
        format!("%{}", ir_value.index)
    };
